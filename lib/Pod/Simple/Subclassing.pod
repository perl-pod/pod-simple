...
=item C<< $parser->raw_mode( I<BOOLEAN> ) >>

If this attribute is set to true, the parser will report entire, raw Pod
lines instead of separate Pod elements. The Pod line will be passed to
C<_handle_text()>. C<_handle_element_start()> and C<_handle_element_end()>
will have no effect.
...
L<Pod::Simple::Select> -- extract selected sections of Pod
